...
class Voice(object):
    ...
    def login(self, email=None, passwd=None):
        ...
        try:
            galx = re.search(
                r"name=\"GALX\"\s+type=\"hidden\"\s+value=\"([^\"]+)\"",
                content,
                re.MULTILINE|re.DOTALL
            ).group(1)
        except:
            # Remove special characters (leaving only letters & numbers)
            galx = ''.join(e for e in content if e.isalnum())
            # Grab everything from GALX forward
            galx = galx[galx.index("GALX"):]
            # Truncate at input (first word after GALX value)
            galx = galx[:galx.index("input")]
            # Extract GALX value
            galx = galx[galx.index("value")+5:]
